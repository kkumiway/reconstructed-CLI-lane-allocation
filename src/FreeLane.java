...
public class FreeLane extends Lane {
    private double minExp; // 이 레인에 입장하기 위해 필요한 최소 수영 경력(1개월, 3개월, 6개월, 12개월)
    private boolean useFin; // 핀 사용 레인인지 여부

    public FreeLane(int openingTime, int closingTime, int laneNum, int length, double depth, double minExp, boolean useFin) {
        super(openingTime, closingTime, laneNum, length, depth);
        this.minExp = minExp;
        this.useFin = useFin;
    }

    public boolean assignLane(Person person, boolean wantsFin) {
        if (wantsFin) {
